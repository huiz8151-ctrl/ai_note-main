...
const LOCAL_PYTHON_CANDIDATES = [
  path.join(ROOT, ".chatbot-python", "Scripts", "python.exe"),
  path.join(ROOT, ".uv-python", "cpython-3.11-windows-x86_64-none", "python.exe")
];
...
function resolvePythonBinary() {
  ...
  const localManaged = LOCAL_PYTHON_CANDIDATES.find((candidate) => fs.existsSync(candidate));
  if (localManaged) {
    return localManaged;
  }
...
async function sendChatbotMessage({ config, message, sessionId }) {
